...
function generateLazyModuleImport(from, to, moduleClassName) {
    ...
    return `() => import('./${core_1.dirname(path)}/${core_1.basename(path)}').then(m => m.${moduleClassName})`;
}
...
function addMissingPaths(tree, routingModulePath, targetFile) {
    ...
    for (let i = 0; i < pathsToCheck.length; i++) {
        ...
        if (!getRouteChildren(route)) {
            utils_1.addObjectProperty(tree, schematics_2.parseSourceFile(tree, routingModulePath), route, 'children: []');
        }
    }
}
...
function addBaseRoute(tree, targetFile) {
    ...
    if (baseRoute) {
        if (!getRouteChildren(baseRoute)) {
            utils_1.addObjectProperty(tree, schematics_2.parseSourceFile(tree, baseModulePath), baseRoute, 'children: []');
        }
        return;
    }
...
function addRoute(tree, routingModulePath, routes, route, componentClass, fileImportPath) {
    const source = schematics_2.parseSourceFile(tree, routingModulePath);
    ...
    if (componentClass && fileImportPath) {
        const importChange = schematics_2.insertImport(source, source.fileName, componentClass, fileImportPath);
