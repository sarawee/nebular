...
function getClassWithDecorator(tree, path, decoratorName) {
    return schematics_1.findNodes(schematics_1.parseSourceFile(tree, path), ts.SyntaxKind.ClassDeclaration)
...
function findDeclarationByIdentifier(source, identifierText) {
    return schematics_1.getSourceNodes(source)
...
function addDeclaration(tree, modulePath, componentClass, importPath) {
    const source = schematics_1.parseSourceFile(tree, modulePath);
    const declarationsChanges = schematics_1.addDeclarationToModule(source, modulePath, componentClass, importPath);
