...
function processService(tree, servicePath) {
    ...
    for (const service of serviceDeclarations) {
        ...
        const source = schematics_2.parseSourceFile(tree, servicePath);
        const changes = schematics_2.addSymbolToNgModuleMetadata(source, modulePath, 'providers', serviceClassName, importString);
...
function addModuleRoute(tree, moduleDeclaration, modulePath, routingModulePath) {
    ...
    const lazyModuleImport = utils_1.generateLazyModuleImport(routingModulePath, modulePath, moduleClassName);
    const loadChildren = `loadChildren: ${lazyModuleImport}`;
    utils_1.addObjectProperty(tree, schematics_2.parseSourceFile(tree, routingModulePath), route, loadChildren);
}
function multilineDeclarationsArray(tree, modulePath) {
    const source = schematics_2.parseSourceFile(tree, modulePath);
    const decoratorNode = schematics_2.getDecoratorMetadata(source, 'NgModule', '@angular/core')[0];
    if (!decoratorNode) {
...
function processRoutingModule(tree, modulePath) {
    ...
    const featureModuleSource = schematics_2.parseSourceFile(tree, featureModulePath);
    ...
    for (const moduleDeclaration of moduleDeclarations) {
        ...
        const changes = schematics_2.addImportToModule(featureModuleSource, featureModulePath, className, importString);
